...
session_start();
// connect to database
$db = mysqli_connect('localhost', 'root', '', 'larustique');
...
function register(){
	...
	if (count($errors) == 0) {
		...
		if (isset($_POST['user_type'])) {
			...
			$query = "INSERT INTO medewerker (username, email, user_type, wachtwoord)
...
function getUserById($id){
	global $db;
	$query = "SELECT * FROM medewerker WHERE medewerkerid=" . $id;
...
function login(){
	...
	if (empty($username)) {
		array_push($errors, "gebruikersnaam is verplicht");
		$_SESSION['wachtwoord_onjuist'] = true;
	...
	if (count($errors) == 0) {
		...
		$query = "SELECT * FROM medewerker WHERE username='$username' AND wachtwoord='$password' LIMIT 1";
